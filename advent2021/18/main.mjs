import * as sf from "./snailfish.mjs";
...
let main = () => {
    //console.log(sf.stringify(doAssignment(assignment)));
    let max = 0;
    for (let i of assignment) {
        for (let j of assignment) {
            let mag = sf.magnitude(sf.add(i, j));
            if (mag > max) {
                max = mag;
            }
        }
